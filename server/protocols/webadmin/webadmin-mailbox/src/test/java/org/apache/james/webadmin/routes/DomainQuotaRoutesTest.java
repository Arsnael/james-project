...
class DomainQuotaRoutesTest {
    ...
    private static final String LOST_COM = "lost.com";
    private static final Domain FOUND_COM = Domain.of("found.com");
    ...
    void getCountShouldReturnNotFoundWhenDomainDoesntExist() {
        when()
            .get(QUOTA_DOMAINS + "/" + LOST_COM + "/" + COUNT)
        .then()
    ...
    void getCountShouldReturnNoContentByDefault() {
        given()
            .get(QUOTA_DOMAINS + "/" + FOUND_COM.name() + "/" + COUNT)
        .then()
    ...
    void getCountShouldReturnStoredValue() throws Exception {
        int value = 42;
        maxQuotaManager.setDomainMaxMessage(FOUND_COM, QuotaCount.count(value));
    ...
    void putCountShouldReturnNotFoundWhenDomainDoesntExist() {
        given()
            .body("123")
        .when()
            .put(QUOTA_DOMAINS + "/" + LOST_COM + "/" + COUNT)
        .then()
            .statusCode(HttpStatus.NOT_FOUND_404);
    }
    ...
    void putCountShouldRejectInvalid() {
        Map<String, Object> errors = given()
            .body("invalid")
            .put(QUOTA_DOMAINS + "/" + FOUND_COM.name() + "/" + COUNT)
        .then()
    ...
    void putCountShouldSetToInfiniteWhenMinusOne() {
        ...
        assertThat(maxQuotaManager.getDomainMaxMessage(FOUND_COM)).contains(QuotaCount.unlimited());
    }
    ...
    void putCountShouldAcceptValidValue() {
        ...
        assertThat(maxQuotaManager.getDomainMaxMessage(FOUND_COM)).contains(QuotaCount.count(42));
    }

    @Test
    void deleteCountShouldReturnNotFoundWhenDomainDoesntExist() {
        when()
            .delete(QUOTA_DOMAINS + "/" + LOST_COM + "/" + COUNT)
        .then()
            .statusCode(HttpStatus.NOT_FOUND_404);
    }
    ...
    void deleteCountShouldSetQuotaToEmpty() throws Exception {
        maxQuotaManager.setDomainMaxMessage(FOUND_COM, QuotaCount.count(42));

        given()
            .delete(QUOTA_DOMAINS + "/" + FOUND_COM.name() + "/" + COUNT)
        .then()
            .statusCode(HttpStatus.NO_CONTENT_204);

        assertThat(maxQuotaManager.getDomainMaxMessage(FOUND_COM)).isEmpty();
    }

    @Test
    void getSizeShouldReturnNotFoundWhenDomainDoesntExist() {
            when()
                .get(QUOTA_DOMAINS + "/" + LOST_COM + "/" + SIZE)
            .then()
                .statusCode(HttpStatus.NOT_FOUND_404);
    }
    ...
    void getSizeShouldReturnNoContentByDefault() {
        when()
            .get(QUOTA_DOMAINS + "/" + FOUND_COM.name() + "/" + SIZE)
        .then()
            .statusCode(HttpStatus.NO_CONTENT_204);
    }
    ...
    void getSizeShouldReturnStoredValue() throws Exception {
        long value = 42;
        maxQuotaManager.setDomainMaxStorage(FOUND_COM, QuotaSize.size(value));
    ...
    void putSizeShouldRejectInvalid() {
        Map<String, Object> errors = given()
            .body("invalid")
            .put(QUOTA_DOMAINS + "/" + FOUND_COM.name() + "/" + SIZE)
        .then()
            .statusCode(HttpStatus.BAD_REQUEST_400)
            .contentType(ContentType.JSON)
            .extract()
    ...
    void putSizeShouldReturnNotFoundWhenDomainDoesntExist() {
        given()
            .body("123")
        .when()
            .put(QUOTA_DOMAINS + "/" + LOST_COM + "/" + SIZE)
        .then()
            .statusCode(HttpStatus.NOT_FOUND_404);
    }
    ...
    void putSizeShouldSetToInfiniteWhenMinusOne() {
        ...
        assertThat(maxQuotaManager.getDomainMaxStorage(FOUND_COM)).contains(QuotaSize.unlimited());
    }
    ...
    void putSizeShouldAcceptValidValue() {
        ...
        assertThat(maxQuotaManager.getDomainMaxStorage(FOUND_COM)).contains(QuotaSize.size(42));
    }

    @Test
    void deleteSizeShouldReturnNotFoundWhenDomainDoesntExist() {
        when()
            .delete(QUOTA_DOMAINS + "/" + LOST_COM + "/" + SIZE)
        .then()
            .statusCode(HttpStatus.NOT_FOUND_404);
    }
    ...
    void deleteSizeShouldSetQuotaToEmpty() throws Exception {
        maxQuotaManager.setDomainMaxStorage(FOUND_COM, QuotaSize.size(42));

        given()
            .delete(QUOTA_DOMAINS + "/" + FOUND_COM.name() + "/" + SIZE)
        .then()
            .statusCode(HttpStatus.NO_CONTENT_204);

        assertThat(maxQuotaManager.getDomainMaxStorage(FOUND_COM)).isEmpty();
    }

    @Test
    void getQuotaShouldReturnNotFoundWhenDomainDoesntExist() {
        when()
            .get(QUOTA_DOMAINS + "/" + LOST_COM)
        .then()
            .statusCode(HttpStatus.NOT_FOUND_404);
    }
    ...
    void getQuotaShouldReturnBothEmptyWhenDefaultValues() {
        JsonPath jsonPath =
            given()
                .get(QUOTA_DOMAINS + "/" + FOUND_COM.name())
            .then()
                .statusCode(HttpStatus.OK_200)
                .contentType(ContentType.JSON)
                .extract()
    ...
    void getQuotaShouldReturnSizeWhenNoCount() throws Exception {
        int maxStorage = 42;
        maxQuotaManager.setDomainMaxStorage(FOUND_COM, QuotaSize.size(maxStorage));
    ...
    void getQuotaShouldReturnBothWhenNoSize() throws Exception {
        int maxMessage = 42;
        maxQuotaManager.setDomainMaxMessage(FOUND_COM, QuotaCount.count(maxMessage));
    ...
    void getQuotaShouldDisplayScopesWhenUnlimited() throws Exception {
        ...
        maxQuotaManager.setDomainMaxMessage(FOUND_COM, QuotaCount.count(maxMessage));
        maxQuotaManager.setDomainMaxStorage(FOUND_COM, QuotaSize.unlimited());
    ...
    void putQuotaShouldUpdateBothQuota() {
        given()
            ...
            .put(QUOTA_DOMAINS + "/" + FOUND_COM.name())
        .then()
            .statusCode(HttpStatus.NO_CONTENT_204);

        assertThat(maxQuotaManager.getDomainMaxMessage(FOUND_COM)).contains(QuotaCount.count(52));
        assertThat(maxQuotaManager.getDomainMaxStorage(FOUND_COM)).contains(QuotaSize.size(42));
    }
