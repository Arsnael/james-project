...
class Serializer @Inject() (mailboxIdFactory: MailboxId.Factory) {
  ...
  private implicit def jsonValidationErrorsWrites(implicit jsonValidationErrorWrites: Writes[JsonValidationError]): Writes[MutableSeq[JsonValidationError]] =
    (errors: MutableSeq[JsonValidationError]) => {
      JsArray(errors.map(error => jsonValidationErrorWrites.writes(error)).toArray[JsValue])
    }

  private implicit def errorsWrites(implicit jsonValidationErrorsWrites: Writes[MutableSeq[JsonValidationError]]): Writes[MutableSeq[(JsPath, MutableSeq[JsonValidationError])]] =
    (errors: MutableSeq[(JsPath, MutableSeq[JsonValidationError])]) => {
      errors.foldLeft(JsArray.empty)((jsArray, jsError) => {
        val (path: JsPath, list: MutableSeq[JsonValidationError]) = jsError
